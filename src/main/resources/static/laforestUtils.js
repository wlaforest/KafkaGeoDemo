...
var nodes = document.getElementsByTagName('td'),
    colors = generateRandomColors(nodes.length);
...
function getColorFromRedToGreenByPercentage(value, max) {
    var newValue = (value*100)/max
    const hue = Math.round(newValue);
    return ["hsl(", hue, ", 50%, 50%)"].join("");
}

function perc2color(iperc,min,max) {
    var base = (max - min);
    perc = Math.abs(iperc - max);
    if (base == 0) { perc = 100; }
    else {
